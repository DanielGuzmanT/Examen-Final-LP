...
void Vehiculo::imprimir(std::ofstream& out) {
    ...
    out << "Lista de infracciones: " << endl;
    list<int>::iterator iter= this->infracciones.begin();
    for(;iter!=infracciones.end();++iter) out << *iter << endl;
}
...
void Vehiculo::insertarInfraccion(int infraccion) {
    this->infracciones.push_back(infraccion);
    this->infracciones.sort();
}
